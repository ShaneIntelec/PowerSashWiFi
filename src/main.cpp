...
void handleEEP() {
 String s = EEP_page; //Read HTML contents
 server.send(200, "text/html", s); //Send web page
}


void handleCSS () {
 String s = CSS_page; //Read HTML contents
...
//    eepString += "<br/>Door Top:" + (String)top;
//    eepString += "<br/>Door Bottom:" + (String)bot;
...
//    dataString += "<br/>Drive State:" + (String)drs;
    ...
    else dataString += "<p><br>Lower Door function disabled while Push Close option disabled</p>";
...
void setup(void){
  ...
  server.on("/eep", handleEEP);       //This is display page
  server.on("/runData", handleRunData);//To get update of run data only
//  server.on("/style2.css", handleRunData);//To get update of run data only
  server.on("/style2.css", handleCSS);
...
void loop(void){
 ...
 if (millis() - timepoll > 1000)
  {
    timepoll = millis();
//    if (digitalRead(BUT1)==0) Serial.println("But 1");
//    #ifdef DEBUGDATA
